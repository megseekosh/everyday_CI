...
```{r, clean speech and voc clips wo speech }
speech <- pre_speech %>%
...
voc_cts <- vocs %>%
  nrow()
pre_voc_cts <- pre_vocs %>%
  nrow() 
```

```{r, compare siblings and household members by group}
matches_methods <- matches %>%
  mutate(num_siblings=as.numeric(num_siblings_excluding_child),
           num_members=as.numeric(num_household_including_child))

aov(num_siblings~match, data=matches_methods) %>%
  summary(.)

aov(num_members~match, data=matches_methods) %>%
  summary(.)
```
...
household <- matches %>% 
  group_by(match) %>%
  summarize(avg_sibs = mean(num_siblings_excluding_child),
            sd_sibs = sd(num_siblings_excluding_child),
            min_sibs = min(num_siblings_excluding_child),
            max_sibs = max(num_siblings_excluding_child),
            avg_mem = mean(num_household_including_child),
            sd_mem = sd(num_household_including_child),
            min_mem = min(num_household_including_child),
            max_mem = max(num_household_including_child)) %>%
  mutate_if(is.numeric, round, 2) %>%
  mutate(Siblings=paste0(avg_sibs, '(', sd_sibs, ')', ',',min_sibs, '-', max_sibs),
         Household_members=paste0(avg_mem,'(',sd_mem,')',',',min_mem,'-',max_mem)) %>%
  select(match, Siblings,Household_members)
...
```{r, family composition tables for supp materials I}
sib_ct <- matches %>% 
  group_by(match,num_siblings_excluding_child) %>%
  count(num_siblings_excluding_child)

birth_ct <- matches %>% 
  group_by(match,birth_order) %>%
  count(birth_order)
```
...
voc_growth_tbl <- its_df %>%
  ...
  group_by(match) %>%
  do(voc_growth = lm(normed_vocs~age_mos_centered, data=.),
     mod2 = cor(.$normed_vocs, .$age_mos_centered, method = "pearson")) %>%
...
# create a 4th "match" of CI kids to compute hearing age for repeated measures analysis
ha_kids_repeat <- vocs %>%
  filter(match=='CI') %>%
  select(-age_mos, -match) %>%
  mutate(age_mos = hearing_age,
...
# the mixed model
voc_dur_nested <-  vocs %>% 
    rbind(., ha_kids_repeat) %>%
    mutate(age_mos_centered = center_scale(age_mos)) %>%
    group_by(match) %>%
    do(lmer_sum(.,formula=childUttLen*1000~age_mos_centered+(1|child_id))) # the mixed model

# p-values
voc_growth_pvalues <- vocs %>% 
  rbind(., ha_kids_repeat) %>%
    mutate(age_mos_centered = center_scale(age_mos)) %>%
    group_by(match) %>%
    do(model = lmer(childUttLen*1000~age_mos_centered+(1|child_id), data=.)) 

# the correlation stat
voc_dur_growth_tbl <- vocs %>%
  rbind(., ha_kids_repeat) %>%
  mutate(age_mos_centered = center_scale(age_mos)) %>%
  group_by(match) %>%
  do(mod2 = cor(.$childUttLen*1000, .$age_mos_centered, method = "pearson")) %>%
  mutate(Pearson = mod2[1]) %>%
  merge(., voc_dur_nested, by='match') %>%
  select(match, age_mos_centered, Pearson) %>%
  mutate_if(is.numeric, round, 2) %>%
  mutate(stats=paste0('B=',age_mos_centered,",","r=", Pearson)) %>%
  select(-age_mos_centered, -Pearson) %>%
  spread(match, stats) %>%
  mutate(measure='Child voc. duration growth') 
...
speech_input_growth_tbl <- its_df %>%
  ...
  select(-slope, -p_value, -Pearson) %>%
  spread(match, stats) %>%
  mutate(measure='Adult speech (s) growth') 
```

## Visualize input

```{r, plot histogram to show indiviudal differences in sound level}
speech %>%
    mutate(match=recode(match,
                      chrono='Chrono. \n age matches',
                      CI='Cochlear implant',
...
```{r, center input variables}
hourly_speech <- hourly_speech %>%
  mutate(age_mos_centered = center_scale(age_mos))
```
...
hourly_speech_m0 <- lmer(normed_hourly_mins~ + (1 | child_id) + (1|hours), data=hourly_speech)
hourly_speech_m1 <- lmer(normed_hourly_mins~ match+ (1 | child_id) + (1|hours), data=hourly_speech)
anova(hourly_speech_m0,hourly_speech_m1)
# does the overall amount of speech change as children age?
hourly_speech_m2 <- lmer(normed_hourly_mins~ age_mos_centered+ (1 | child_id) + (1|hours), data=hourly_speech)
...
# does the overall amount of speech change as children age?
hourly_mins_m2 <- lmer(normed_hourly_words~ age_mos_centered+ (1 | child_id) + (1|hours), data=hourly_speech)


# ---------- CONSISTENCY ------------
input_consis2 <- its_df %>% select(age_mos, child_id) %>% merge(., input_consis, by='child_id')

input_consis2 <- input_consis2 %>%
  mutate(age_mos_centered = center_scale(age_mos))

input_consis2$match <- relevel(factor(input_consis2$match), ref = "CI")
speech_consis_m0 <- lm(perc_words~age_mos_centered, data=input_consis2)
speech_consis_m1 <- lm(perc_words~ age_mos_centered + match, data=input_consis2)
anova(speech_consis_m0,speech_consis_m1) # no
speech_consis_m2 <- lm(perc_words~ age_mos_centered*match, data=input_consis2)
...
convo_consis_m2 <- lm(perc_turns~ age_mos*match, data=convo_consis_check2)
...
# first filter to only get child vocs that were directly contingent with an adult 
contingent_df_lag <- contingent_df %>%  
  filter(contingent=='Y' & segment_type=='CHN' & (lag(segment_type=='FAN')|lag(segment_type=='MAN'))) 
...
for_temp <- vocs %>% 
  ...
  mutate(clip_onset = onset,
         clip_offset = offset) %>%
  select(-offset, -onset)

pre_time_vocs <- speech %>%
...
intens <- contingent_df %>%
  ...
  group_by(child_id) %>%
  mutate(adult_loudness = if_else(avg_dB>med_dB, "louder", "softer"))
...
all_intens_vocs <- intens %>%  
  ...
  mutate(child_voc_dB=avg_dB) %>% # classify the dB as child dB because we're going to filter for that
...
ggplot(., aes(age_mos, perc_contingent)) +
  ...
  ylab("Percentage of \n contingent vocalizations") + 
    theme(axis.title = element_text(face ="bold", size=12),
        ...
        axis.text = element_text(face="bold", color='gray50', size=9),
        strip.text=element_text(face='bold', size=10))
ggsave("../noise_pilotData/CI_ICPhS/CI_ICPhS_results_files/contingent_age.pdf", width = 8, height = 3)
...
```{r, model contingent vocalizations, eval=FALSE}
# ------- look at percentage contingent -------
# look at just three groups
contingent_final_3 <- contingent_final %>% 
  filter(match!='CI_by_hearing_age') %>%
  mutate(age_mos_centered=center_scale(age_mos),
         hearing_age_centered=center_scale(hearing_age))
contingent_final_3$match <- relevel(factor(contingent_final_3$match), ref = "HA")
...
ci_for_model <- contingent_final_3 %>% filter(match=='CI')
ci_cor <- cor.test(ci_for_model$age_mos_centered, ci_for_model$perc_contingent)
ci_cor_ha <- cor.test(ci_for_model$hearing_age_centered, ci_for_model$perc_contingent)
```
...
row.names(meas_mat) <- c("Recording duration (hrs.)",
                           ...
                           #"Input:output", # maybe include this?
                           #"% contingent vocs.",
                           "Convo. turn quantity",
...
## Create audiological measures table
```{r, create aud table}

matches %>%
  filter(match=='CI') %>%
  select(child_id, age_mos_at_recording, age_hearing_lost_mos, age_of_implantation, hearing_age, etiology, device_config, activation) %>%
  mutate(age_hearing_lost_mos=as.factor(age_hearing_lost_mos)) %>%
kable(., booktabs=T, 
              caption= "Audiological information from children with cochlear implants",
              col.names = c("Participant", 
                            "Chronological age", 
                           "Age at hearing loss (mos)", 
                           "Age at activation", 
                           "Hearing age (since implantation)",
                           "Etiology",
