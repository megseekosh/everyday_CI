---
title: "Replicating results for 'The everyday speech environments of preschoolers with
and without cochlear implants' after removing the child activated at 45 mos"
author: "Meg Cychosz"
...
knitr::opts_knit$set(root.dir = 'Library/CloudStorage/Box-Box/box-group-lena-studies/noise_pilotData/everyday_CI/')
```
...
num_CI <- pre_vocs %>% 
  ...
  filter(match=='CI') %>%
  nrow()
print(paste('There should be 17 children w/ CIs and there are', num_CI))
...
time_steps <- rep(seq(60,57600,60),times=51) %>% as.data.frame() 
...
# duration
vocs$match <- relevel(factor(vocs$match), ref = "chrono")
...
time_steps <- rep(seq(1,192,1),times=51) %>% as.data.frame() 
...
anova(convo_consis_m0,voc_consis_m2) 
```
...
contingent_final_3$match <- relevel(factor(contingent_final_3$match), ref = "CI")
contingent_m0 <- lm(perc_contingent~match, data=contingent_final_3)
contingent_m1 <- lm(perc_contingent~age_mos_centered, data=contingent_final_3)
contingent_m2 <- lm(perc_contingent~hearing_age_centered, data=contingent_final_3)
contingent_m3 <- lm(perc_contingent~match*age_mos_centered, data=contingent_final_3)

# following median split
binary_intens_m0 <- lmer(perc_contingent_loudness~ + (1 | child_id), data=intens2)
binary_intens_m1 <- lmer(perc_contingent_loudness~adult_loudness + (1 | child_id), data=intens2)
anova(binary_intens_m0,binary_intens_m1) # yes 
```
